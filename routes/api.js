...
// Enhanced authMiddleware to check both Authorization Header and HTTP Cookies
function authMiddleware(req, res, next) {
  ...
  if (authHeader && authHeader.startsWith('Bearer ')) {
    ...
    // Parse cookies safely to look for token or jwt
...
function setupRoutes(app, db) {
  ...
  app.post('/api/auth/register', async (req, res) => {
    try {
      ...
      // Set secure HTTP cookie for seamless session persistence
      ...
      res.json({ user: { id: user.id, email: user.email, fullName: user.full_name, tier: user.tier, virtualBalance: user.virtual_balance, isAdmin: user.is_admin }, token, redirect: user.is_admin ? '/admin' : '/dashboard' });
    } catch (err) {
  ...
  app.get('/api/admin/dashboard/stats', authMiddleware, adminMiddleware, async (req, res) => {
    try {
      let stats;
      if (typeof db.getAdminStats === 'function') {
        stats = await db.getAdminStats();
      } else {
        const users = await db.getAllUsers();
        const nonAdminUsers = users.filter(u => !u.is_admin);
        const totalVirtualBalance = nonAdminUsers.reduce((acc, u) => acc + (u.virtual_balance || 0),);
        stats = {
          totalUsers: nonAdminUsers.length,
          totalVirtualBalance,
          totalTransactions: 0,
          blockedWithdrawals: 0,
          activeAssets: (await db.getAllAssets()).length,
          tierDistribution: []
        };
      }
      res.json({ stats });
